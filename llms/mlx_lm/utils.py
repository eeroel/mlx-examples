...
def lookup_generate_step(
    ...
    *,
    ...
    Args:
        prompt (mx.array): The input prompt.
        model (nn.Module): The model to use for generation.
        prediction (mx.array): The tokens to use for lookup decoding.
    ...
    lookup_pos = 0
    lookup_wsize = 1
    pred = prediction  # TODO we want to mutate this, OK?
    # Create the KV cache for generation
    if prompt_cache is None:
    ...
    def _draft_generate(pred, y, lookup_pos, num_draft):
        if num_draft == 0 or prev_tokens is None:
            mx.array([], mx.uint32), lookup_pos
        if pred[lookup_pos - 1] == y[-1]:
            return pred[lookup_pos:lookup_pos + num_draft], lookup_pos
        else:
            # TODO we sometimes spend a nontrivial amount of time here => 
            # better data structure?
            lpred = pred.tolist()
            try:
                idx = lpred.index(y[-1]) + 1
                return pred[idx:idx + num_draft], idx
            except ValueError:
                pass
        return mx.array([], mx.uint32), lookup_pos
    ...
    try:
        while True:
            num_draft = min(max_tokens - ntoks, lookup_wsize)
            draft_tokens, lookup_pos = _draft_generate(pred, y, lookup_pos, num_draft)
...
def stream_generate(
    ...
    **kwargs,
    ...
    if draft_model is None and prediction is not None:
        kwargs.pop("num_draft_tokens", None)
        if not isinstance(prediction, mx.array):
            if isinstance(prediction, str):
                # Try to infer if special tokens are needed
                add_special_tokens = tokenizer.bos_token is None or not prediction.startswith(
                    tokenizer.bos_token
                )
